	...
	if (this != NULL && nodeIndex >= 0 && nodeIndex < longitud) {
		...
		if (node != NULL) {
			for (int i = 0; i < nodeIndex; i++) {
				node = node->pNextNode; //Todavia no se encontró, así que lo asigno a node
			}
		}
	}
	return node;
	...
	if (node != NULL) {
		if (this != NULL && nodeIndex <= longitud && nodeIndex >= 0) {
			if (nodeIndex == 0) {
				...
				Node *auxNode = getNode(this, nodeIndex - 1); //auxNode->pNextNode = Nodo 4
				node->pNextNode = auxNode->pNextNode; //node->pNextNode = Nodo 4
...
int ll_remove(LinkedList *this, int index) {
	...
	int longitud = ll_len(this);
	if (this != NULL && index >= 0 && index < longitud) {
		Node *nodeToRemove = getNode(this, index);
...
int ll_sort(LinkedList *this, int (*pFunc)(void*, void*), int order) {
	int returnAux = -1;
	void *pFirstElement = NULL;
	void *pSecondElement = NULL;
	int criterio;
	int longitud = ll_len(this);
	...
	if (this != NULL && pFunc != NULL && (order == 0 || order == 1)) {
		for (int i = 0; i < longitud - 1; i++) {
			for (int j = i + 1; j < longitud; j++) {
				...
				criterio = pFunc(pFirstElement, pSecondElement);
